			...
			<div className="md:col-start-3 xl:col-start-4 md:col-span-3 space-y-36">
					...
					<div className="space-y-4">
						{projects.map((project) => {
							const isInternal = Boolean(
								project.slug && project.sections?.length,
							);

							return (
								<Link
									key={project.name}
									href={isInternal ? `/c/${project.slug}` : project.url || "#"}
									{...(!isInternal
				...
				<div className="space-y-12 max-md:max-w-md">
					...
					<p className="space-y-4">
						<a
							className="block text-foreground-muted hover:underline hover:text-white transition-colors duration-300"
